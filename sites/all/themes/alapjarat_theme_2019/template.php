...
// Add Zen Tabs styles.
if (theme_get_setting('alapjarat_theme_2019_tabs')) {
  drupal_add_css(drupal_get_path('theme', 'alapjarat_theme_2019') . '/css/tabs.css');
}

/**
 * Implements hook_preprocess_html().
 */
function alapjarat_theme_2019_preprocess_html(&$variables) {
  global $user, $language;

  // Add role name classes (to allow css based show for admin/hidden from user).
  foreach ($user->roles as $role) {
    $variables['classes_array'][] = 'role-' . alapjarat_theme_2019_id_safe($role);
  }
  ...
  if (!$variables['is_front']) {
    ...
    $variables['classes_array'][] = alapjarat_theme_2019_id_safe('page-' . $path);
    $variables['classes_array'][] = alapjarat_theme_2019_id_safe('section-' . $section);
  ...
  // Add IE classes.
  if (theme_get_setting('alapjarat_theme_2019_ie_enabled')) {
    $alapjarat_theme_2019_ie_enabled_versions = theme_get_setting('alapjarat_theme_2019_ie_enabled_versions');
    if (in_array('ie8', $alapjarat_theme_2019_ie_enabled_versions, TRUE)) {
    ...
    if (in_array('ie9', $alapjarat_theme_2019_ie_enabled_versions, TRUE)) {
 ...
 */
function alapjarat_theme_2019_preprocess_page(&$variables, $hook) {
 ...
 */
function alapjarat_theme_2019_preprocess_node(&$variables) {
  ...
  // Merge first/last class (from alapjarat_theme_2019_preprocess_page) into classes array of
  // current node object.
  $node = $variables['node'];
 ...
 */
function alapjarat_theme_2019_preprocess_block(&$variables) {
 ...
 */
function alapjarat_theme_2019_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('alapjarat_theme_2019_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('alapjarat_theme_2019_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('alapjarat_theme_2019_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('alapjarat_theme_2019_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('alapjarat_theme_2019_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function alapjarat_theme_2019_id_safe($string) {
 ...
 */
function alapjarat_theme_2019_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = alapjarat_theme_2019_id_safe($element['#title']);
 ...
 */
function alapjarat_theme_2019_preprocess_menu_local_task(&$variables) {
 ...
 */
function alapjarat_theme_2019_menu_local_tasks(&$variables) {
  $output = '';
...
function alapjarat_theme_2019_page_alter(&$page)
{
  if (arg(0) == 'search')
  {
